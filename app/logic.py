...
BASIC_MILESTONES = [
    Milestone(
        ...
        resources=[Resource(title="Mode: SQL Tutorial", url="https://mode.com/sql-tutorial/",
                            type="course")],
        estimated_hours=6,
        tags=["core"]
    ),
    Milestone(
        ...
        resources=[Resource(title="PostgreSQL docs: SELECT", url="https://www.postgresql.org/docs/current/queries-select.html", type="doc")],
        estimated_hours=6,
        tags=["core"]
    ),
    Milestone(
        ...
        resources=[Resource(title="CTE Explained", url="https://www.sqltutorial.org/sql-cte/", type="article")],
        estimated_hours=5,
        tags=["core"]
    ),
    Milestone(
        ...
        resources=[Resource(title="Use The Index, Luke!", url="https://use-the-index-luke.com/", type="book")],
        estimated_hours=6,
        tags=["perf"]
    ),
    Milestone(
        ...
        resources=[Resource(title="PostgreSQL Concurrency Control", url="https://www.postgresql.org/docs/current/mvcc.html", type="doc")],
        estimated_hours=4,
        tags=["db-theory"]
    ),
    Milestone(
        ...
        resources=[Resource(title="PostgreSQL: Window Functions", url="https://www.postgresql.org/docs/current/functions-window.html", type="doc")],
        estimated_hours=5,
        tags=["analytics"]
    ),
    Milestone(
        ...
        resources=[Resource(title="Intro to Data Modeling", url="https://www.kimballgroup.com/data-warehouse-business-intelligence-resources/", type="article")],
        estimated_hours=4,
        tags=["modeling"]
    ),
    Milestone(
        ...
        resources=[Resource(title="SQLBolt Exercises", url="https://sqlbolt.com/", type="exercise")],
        estimated_hours=6,
        tags=["practice"]
    ),
]
...
def _infer_level_from_text(blobs: List[str]) -> str:
    ...
    if any(k in text for k in ["window function", "окн", "explain analyze", "индекс", "изоляц"]):
        return "intermediate"
    ...
    if "postgres" in joined_text or "postgresql" in joined_text:
        for m in milestones:
            if m.id in {"indexes-perf", "window-funcs"}:
                m.resources.append(Resource(title="PG: EXPLAIN", url="https://www.postgresql.org/docs/current/using-explain.html", type="doc"))
    if "mysql" in joined_text:
        for m in milestones:
            if m.id == "indexes-perf":
                m.resources.append(Resource(title="MySQL Optimizer", url="https://dev.mysql.com/doc/refman/8.0/en/optimizer-statistics.html", type="doc"))
        ...
        for m in roadmap.milestones:
            if m.id in {"indexes-perf", "window-funcs"}:
                if not any("postgresql" in (r.url or "") for r in m.resources):
                    m.resources.append(Resource(title="PostgreSQL Docs", url="https://www.postgresql.org/docs/current/", type="doc"))
    if "mysql" in p:
        for m in roadmap.milestones:
            if m.id == "indexes-perf":
                if not any("mysql" in (r.url or "") for r in m.resources):
                    m.resources.append(Resource(title="MySQL 8.0 Reference", url="https://dev.mysql.com/doc/refman/8.0/en/", type="doc"))
    ...
    if "добавь ресурсы" in p or "more resources" in p or "extra resources" in p:
        for m in roadmap.milestones:
            m.resources.extend([
                ...
                Resource(title="w3schools SQL", url="https://www.w3schools.com/sql/", type="doc"),
            ])
