...
class BuildMapPipeline:
    ...
    def __init__(self, client: AsyncOpenAI, model: str, model_lite: str):
        ...
        self._lite_sem: asyncio.Semaphore = asyncio.Semaphore(10)
    ...
    async def _link_related(
        self, hierarchy: ConceptHierarchy, language: str
    ) -> RelatedConcepts:
        concepts = flatten_hierarchy(hierarchy)

        messages = related_concepts_prompt(
            concepts=list(concepts), response_model=RelatedConcepts, language=language
        )
    ...
    async def _get_description(
        ...
        related_concepts = related.related.get(concept)
        # TODO: add caching
        parent_concepts = get_parents_map(hierarchy).get(concept)

        messages = add_description_prompt(
            material=material,
            concept=concept,
    ...
    async def build(self, material: dict[str, str], language: str = 'ru'):
        ...
        related = await self._link_related(hierarchy.hierarchy, language=language)
        concepts = list(flatten_hierarchy(hierarchy.hierarchy))

        descriptions = await asyncio.gather(
            *(
                self._get_description(material, concept, related, hierarchy.hierarchy, language)
                for concept in concepts
            )
        )
        description_map = dict(zip(concepts, descriptions))
            ...
            for title, node_data in nodes.items():
                ...
                concept = Concept(
                    ...
                    description=description_map[
                        title
                    ],  # This field is not generated by the current pipeline.
